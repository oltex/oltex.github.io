...
싱글톤은 사실 굉장히 신중히 사용해야하거나 애초에 사용을 지양해야 한다고 합니다.<br>
...
<br>
첫 번째 문제를 보면 컴파일러는 같은 결과를 보장하는 한해서 코드를 훨씬 빠르게<br>
재배치(reordering)하는 기능이 존재하기 때문에 코드의 순서가 바뀔 수 있습니다.<br>
문제는 이것이 멀티 스레드를 고려하지 않는다는 것이죠<br>
new 키워드도 이러한 최적화 과정이 존재합니다. new 키워드는 한줄이지만 3가지 단계로 구성되어있습니다.
...
```
이방식은 멀티 스레드 문제에서 안전함을 보장하기 때문에(Thread-safe) 이른 초기화가 게으른 초기화보다 좋은 상황이거나<br>
싱글톤 클래스의 크기가 크지않다면 고려해볼만한 방법입니다.
### LazyHolder
