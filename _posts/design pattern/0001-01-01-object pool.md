...
### 메모리
오브젝트 풀은 메모리의 할당/해제를 직접 관리하기 위한 패턴입니다.
따라서 오브젝트 풀의 크기를 조절하는 것이 중요합니다.

만약 오브젝트 풀이 너무 작다면 오브젝트 생성에 문제가 발생할 것입니다.
반대로 오브젝트 풀을 너무 크게 설정한다면 메모리를 낭비한다는 소리가 됩니다.

### 예외
앞서 말했듯이 오브젝트 풀의 크기를 잘 조정했다 하더라도
이는 예측에 의한 것이기 때문에 오브젝트 풀의 개수를 초과해 요청이 들어올 경우가 존재할 것입니다.

모든 오브젝트가 active 상태인데 create요청이 들어올 경우를 대비하여야합니다.
...
- 이런 일이 아예 생기지 않게 합니다: 위 예시랑 맞지 않긴 하지만 어떤 오브젝트 풀의 경우<br>
최대치를 절대적으로 예측할 수 있습니다. 예를 들어 총알의 경우 플레이어당 100발이 주어진다면<br>
플레이어 x 100 으로 총알의 최대치를 예측 가능합니다.
### 커플링
