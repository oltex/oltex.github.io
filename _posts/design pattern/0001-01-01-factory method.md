...
### 추상 팩토리
추상 팩토리 패턴과 팩토리 패턴의 차이점에 대해 설명하겠습니다.<br>
<br>
팩토리 메서드의 주된 개념은 메서드라는 점을 놓치면 안됩니다.<br>
...
(추상 팩토리에서는 Creator를 AbstractFactory라고 명명합니다.)<br>
<br>
사실 위쪽 코드에서 Creator는 Monster를 생성하기 위해 만들어진 추상 팩토리에 가까운 느낌이라 햇갈릴 것입니다.<br>
...
그 중 스폰 함수는 몬스터를 스폰하는 함수로 팩토리 메서드를 호출합니다.<br>
<br>
이 코드의 핵심은 Dungeon클래스는 오롯이 Monster를 생성하기 위한 클래스가 아니라는 것입니다.<br>
던전 클래스에서 다른 던전마다 생성되는 몬스터를 다르게 하기위한 방식이 필요했고<br>
그 방법으로 팩토리 메서드 함수를 사용했습니다.<br>
<br>
팩토리 메서드는 추상 펙토리와 다르게 클래스가 팩토리만을 위한게 아닙니다.<br>
단지 팩토리 메서드가 필요한 시점에 함수를 만든거라고 보면됩니다.<br>
<br>
추상 팩토리는 이와 다르게 팩토리 메서드를 사용하기 위해 만들어집니다.<br>
팩토리 메서드를 써야 하는데 그걸 위해서 클래스를 만든 것이지요.<br>
즉,
1. 팩토리 메서드는 함수로써 어디에서 어떻게 사용되든 자유이고 구조도 다양하지만
1. 추상 팩토리는 객체로 팩토리 메서드를 사용하기위해 만들어지며 사용법이 제한적인 클래스
