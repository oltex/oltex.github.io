...
데이터가 n개일 때, 최악의 경우에 해당하는 연산 횟수는 n이다.<br>
<br>
...
답이 딱 나오지 않습니다.<br>
<br>
이진 탐색 트리의 경우 시간 복잡도가 어렵기 때문입니다.<br>
따라서 직접 한번 세어보겠습니다.
...
```
어라 무언가 빠진것 같습니다.<br>
원래라면 log2n + 1이 되야하지 않을까요?<br>
<br>
아쉽게도 그렇지 않습니다.<br>
시간 복잡도를 계산하는 이유에 대해 앞에서 언급했던 내용을 가져와 보겠습니다.<br>
"데이터의 개수에 따른 연산 횟수의 변화 정도를 판단할 수 있습니다."<br>
<br>
데이터 n개에 따른 연산결과중 +1은 변하지 않는 부분입니다.<br>
따라서 시간 복잡도에는 해당 결과를 집어넣지 않습니다.
> ## 본론

드디어 빅오 표기법에 대해 이야기 할 수 있게 되었습니다.<br>
<br>
사실 빅오 표기법에 대해서는<br>
사전 내용에서 거의 설명한 것과 다름이 없습니다.<br>
위 시간 복잡도 내용에서 몇가지만 변화가 있을 뿐입니다.<br>
<br>
다음과 같은 시간 복잡도가 있다고 가정해 보겠습니다.
```
T(n) = n^2 + 2n + 1
```
자 여기서 +1을 빼는 수순을 밟을 수 있을 것입니다.<br>
하지만 2n은 아직 존재하고 있습니다.<br>
<br>
이것을 어떻게 해야 할까요?<br>
기존 시간 복잡도 방식이라면 2n도 결국 n의 영향을 받는<br>
연산 횟수의 변화 정도이니 제외하지 않는것이 맞지만<br>
<br>
빅오 표기법에서는 이를 과감히 제외해버립니다.<br>
<br>
빅오 표기법은 함수에서 가장 영향력이 큰 부분을 찾는 방식입니다.<br>
영어로 얘기하자면 Big O 즉 가장 큰 O를 따지는 것입니다. (여기서 O는 영향력을 의미합니다.)<br>
<br>
시간 복잡도에서 차지하는 영향력이 큰 부분을 제외한 모든 부분을 날려버립니다.<br>
따라서 위 시간 복잡도를 빅오 표기법으로 날려버리면 다음과 같습니다.
```
T(n) = n^2
```
2n를 날려버렸습니다.<br>
위 과정에서 T(n) = n^2 + 2n에서 n^2가 차지하는 비율을 확인해 보겠습니다.
n|n^2|2n|n^2의 비율
---|---|---|---
10|100|20|83.33%
100|10000|200|98.04%
1000|1000000|2000|99.80%
