...
main함수에서 호출하는 함수 템플릿의 \<\>안에 들어가는 값을 가리켜 형식 인수라고 부릅니다.
...
void main(void) {
	std::cout << calc_num<int>(2) << std::endl; //결과: 4
	std::cout << calc_num<float>(0.1f) << std::endl; //결과: 0.01f 난 0.2를 원한다!
}
```
...
void main(void) {
	std::cout << calc_num<int>(1) << std::endl;
	std::cout << calc_num<float>(0.1f) << std::endl;
}
```
