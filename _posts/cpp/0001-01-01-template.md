...
main함수에서 호출하는 함수 템플릿의 \<\>안에 들어가는 형식을 가리켜 형식 인수라고 부릅니다.
...
```
> ## 특수화
템플릿 특수화는 특정 템플릿의 인스턴스를 다르게 정의하는 문법입니다.
...
```
템플릿의 특수화는 크게 2가지 방식이 있습니다. 완전 특수화와 부분 특수화 입니다.
#### 완전 특수화
함수 템플릿의 매개 변수를 전부 특수화 한 경우입니다.
...
```
다음은 클래스의 특수화를 진행해보겠습니다.
위의 코드를 클래스로 변환시킨 버전입니다.
```cpp
template<typename T>
class myclass {
public:
