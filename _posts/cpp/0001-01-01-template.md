...
public:
	myclass(T t) {
		m_num = t + t; //myclass<T> 객채 생성시 여기가 호출된다.
	}
private:
	T m_num;
};
...
//맴버 함수: 템플릿 키워드는 없지만 형식 인수는 작성해야합니다.
myclass<float>::myclass(float t) {
	m_num = t * t; //myclass<float> 객채 생성시 여기가 호출된다.
}
```
...
1번과 2번의 경우 클래스 템플릿을 특수화 한 경우 라고 볼 수 있지만 영향받는 대상이 맴버 함수이기 때문에<br>
맴버 함수의 경우로 묶었습니다.
```cpp
template<typename T>
class myclass {
