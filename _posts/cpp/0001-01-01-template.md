...
위 코드는 min_num\<int\>, min_num\<float\>, min_num\<double\> 의 템플릿 인스턴스가 이루어졌습니다.<br>
...
한 가지 다른점이 함수랑 다르게 암시적 템플릿을 허용하지 않습니다. 꼭 <형식, 값>을 붙여 명시적으로 알려주어야 합니다.<br>
<br>
...
```
이렇게 함수가 클래스 내부에 있는경우 문제 없지만 함수의 외부에 있는 경우 템플릿 키워드를 작성해 줘야합니다.
```cpp
template<typename T, int size>
class vtclass {
	...
...
```
이번엔 맴버 함수가 다른 템플릿을 받는 함수인 경우입니다.
```cpp
template<typename T, int size>
class vtclass {
	...
...
```
일반적인 함수 템플릿과 구현 방법이 비슷합니다만 구현부를 나누면 조금 복잡해집니다.<br>
구현부를 나눈 코드는 다음과 같습니다.
```cpp
template<typename T, int size>
class vtclass {
	...
...
void main(void) {
	vtclass<int, 10> vtc;
	std::cout << vtc.add(0, 10.f) << std::endl;
}
```
템플릿 키워드를 2번 사용하여 구현해줘야합니다.
> ## 특수화

템플릿 특수화는 특정 템플릿의 인스턴스를 다르게 정의하는 문법입니다.<br>
아래와 같은 코드에서 만약 float형이 들어왔을 때<br>
따로 \* 연산을 진행하고 싶다하면 특수화를 통해 다른 처리를 할 수 있습니다.
```cpp
template <typename T>
T calc_num(T t) {
	return t + t;
...
template <>
float calc_num<float>(float t) { //float형 완전 특수화 템플릿입니다.
	return t * t;
}
```
다음은 클래스의 특수화를 진행해보겠습니다.<br>
...
public:
	myclass(const T& t) {
		m_num = t + t; //myclass<int> 객치 생성시 여기가 호출된다.
	}
private:
	T m_num;
};
...
```
마지막으로 맴버 함수 템플릿입니다.<br>
위쪽에서 봤던 T와 U를 사용하여 멤버 함수에 대한 케이스를 구현하겠습니다.<br>
일반 템플릿은 햇갈림을 방지하기 위해 클래스 선언부에 넣었습니다.
...
```
1. 생성자는 템플릿 특수화로 float를 받았을 때 제곱합니다.
1. myclass<float>일 때 calc_num 함수는 템플릿 특수화로 곱셈연산을 수행합니다.
1. calc_num<float> 일 때 템플릿 특수화로 곱셈연산을 수행합니다. (외부에서 선인이 안됩니다.)
1. myclass<float> 이고 calc_num<float> 일 때 템플릿 특수화로 곱셈연산을 수행합니다.
```cpp
template<typename T>
class myclass {
public:
...
template<> //1. 생성자 템플릿 특수화
myclass<float>::myclass(const float& t) {
	m_num = t * t;
}
template<typename T>
template<typename U>
T myclass<T>::calc_num(U u) {
	return m_num + u;
}
template<>  //2. T의 템플릿 특수화
template<typename U>
...
template<>
template<>
float myclass<float>::calc_num(float u) { //만약 myclass도 float고 매개 변수도 float라면 이쪽으로 들어오게 됩니다.
	return m_num * u;
}

template<typename T>
template<>  //에러! U만의 템플릿 특수화는 불가능합니다.
