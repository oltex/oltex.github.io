...
```cpp
template<typename T, typename U> //1번
void calc_num(T t, U u) {
	std::cout << t + u << std::endl;
}
template<typename T> //2번
...
하지만 함수의 호출부를 명시적으로 바꿔보면 다르다는 것을 알 수 있습니다.
(미리 붙여둔 번호를 참고하시기 바랍니다.)
```cpp
void main(void) {
	calc_num<int, float>(10, 10.f); //1번 함수 템플릿이 호출됩니다. 만약 부분 특수화라면 이 경우에 2번이 호출되야 합니다.
	calc_num<int>(10, 10.f); //2번 함수 템플릿 오버로딩이 호출됩니다.
}
```
...
그런 기본 함수 템플릿들이 여러개 있는 경우 매개변수 형식들이 가장 잘 일치하며 "가장 구체적인" 것이 선택되는 데 이때 다음의 난해한 규칙들이 적용됩니다.
  ...
  - "가장 구체적인" 기본 함수 템플릿들이 여러 개인 경우에는 어느 것이 더 적합한지를 컴파일러가 판단할 수 없으므로, 호출은 '애매한' 것이 됩니다. 프로그래머는 어떤 것을 호출하고자 하는지 좀 더 명시적으로 지정해야 합니다.
...
중요한 핵심은 오버로딩 해소 과정에서 "템플릿 특수화는 참여 되지 않는다"는 것입니다.
조금 더 이해를 돕기 위해 아래 몇가지 예제를 참고해보겠습니다.
```cpp
...
void main(void) {
  ...
  calc_num<>(10, 10.f); //5. 함수 템플릿 오버로딩 호출
}
```
위 코드를 실행할 시 호출되는 함수는 각각 2번과 4번입니다.<br>
이로 인해 오버로딩된 함수와 함수 템플릿이 우선순위를 가지는 예를 작성해보았습니다.
추가적으로 설명해야될 사항이 있는데, alc_num<>호출에서는 <>로 인하여 템플릿을 호출한다는것이 명시적으로 표현되었기 때문에 일반 함수 호출은 하지 않습니다.

두번째 예제는 함수 템플릿 완전 특수화가 껴있을 때입니다.
...
```
위 코드에서 3. 함수 템플릿 완전 특수화가 호출된 이유가 단순히 int float 형이기 때문이 아닙니다.
코드의 순서를 바꿔보겠습니다.
```cpp
template<typename T, typename U>
void calc_num(T t, U u) { //1. 함수 템플릿
...
```
단순히 코드의 순서만 바꿨을 뿐인데 호출되는 함수가 2. 함수 템플릿 오버로딩으로 바뀌었습니다.
두 코드의 차이점은
첫 번째 코드는 3. 함수 템플릿 완전 특수화가 2. 함수 템플릿 오버로딩에 대한 거였고
두 번째 코드는 3. 함수 템플릿 완전 특수화가 1. 함수 템플릿에 대한 것이라는 겁니다.
즉 함수 템플릿 완전 특수화가 함수의 오버로딩 해소과정에서 참여되지 못하였다가
함수 템플릿이 선택 된 후 존재한다면 특수화를 진행한 것입니다.
> ## 가변인자 템플릿
