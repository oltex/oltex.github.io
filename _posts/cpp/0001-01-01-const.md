...
#### 상수 객체
클래스 객체 자체에도 const를 사용할 수 있습니다. 이는 상수 객체라고 하며 상수 객체와 상수 멤버 함수와의 관계가 존재합니다.<br>
상수 객체는 해당 클래스 내의 상수 멤버 함수만 사용할 수 있습니다. 또한 멤버 변수를 전부 상수화 시킵니다.<br>
상수 멤버 함수와 멤버 함수는 오버로딩이 가능합니다. 일반 객체는 멤버 함수를 호출하고, 상수 객체는 상수 멤버 함수를 호출하게 됩니다.
```cpp
class TextBlock {
  ...
public:
...
private:
	std::string m_strText;
	mutable bool m_bLengthIsValid = false; //비트수준 상수성제약을 풀어주는 키워드입니다.
...
### 코드 중복
상단의 TextBlock의 operator[]함수를 오버로딩 해서 분리해놨던 코드를 가져와 보겠습니다.
이 코드는 특정 문자의 참조자만 반환하고 있지만
경계 검사, 접근정보 로깅, 무결성 검증의 코드를 추가 했다고 가정해보겠습니다.
```cpp
class CTextBlock {
  ...
public:
	...
	const char& operator[](std::size_t iPos) const {
		... //경계 검사
		... //접근 정보 로깅
		... //무결성 검증
		return m_strText[iPos];
...
```
이런저런 코드들을 모두 상수/비상수 버전에 넣어버린다면 코드 중복이 발생하게 됩니다.
이러한 상황은 컴파일 시간, 유지보수, 코드 비대화등의 문제를 야기할 수 있습니다.
...
public:
	char& operator[](std::size_t iPos) {
		return const_cast<char&>(static_cast<const CTextBlock&>(*this)[iPos]);
	}
	const char& operator[](std::size_t iPos) const {
		... //경계 검사
		... //접근 정보 로깅
...
```
하나하나 풀어서 설명해보면
 1. *this를 const CTextBlock타입으로 형변환 한 후
 1. 상수 멤버 함수의 operator[]를 호출하여 const char&를 리턴받습니다.
