...
#### 상수 객체
클래스 객체 자체에도 const를 사용할 수 있습니다. 이는 상수 객체라고 하며 상수 객체와 상수 멤버 함수와의 관계가 존재합니다.
상수 객체는 해당 클래스 내의 상수 멤버 함수만 사용할 수 있습니다. 또한 멤버 변수를 전부 상수화 시킵니다.
상수 멤버 함수와 멤버 함수는 const의 여부로 오버로딩이 가능합니다. 일반 객체는 멤버 함수를 호출하고, 상수 객체는 상수 멤버 함수를 호출하게 됩니다.
```cpp
class TextBlock {
  ...
public:
