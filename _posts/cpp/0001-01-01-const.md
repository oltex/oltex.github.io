...
const int iValue = 10;
```
한번 선언된 상수는 대게 프로그램 종료시까지 값이 변경될 수 없습니다.
...
### 함수
기본적으로 함수의 반환값과 매개변수에 const를 사용할 수 있습니다. 이러한 상수의 의미는 변수와 똑같습니다.<br>
단 변수와 다르게 초기화 해 줄 필요가 없습니다.
```cpp
...
```
### 클래스
클래스 내의 멤버 변수에 대해 const 키워드를 붙여 멤버 상수로 만들 수 있습니다.<br>
...
상수 맴버 함수가 제한하는 부분은 여러가지가 있는데
 ...
 1. 함수의 반환에 const가 붙지않으면 맴버 변수의 반환을 허용하지 않는다.<br>

한마디로 함수 내부에서 멤버의 변경을 허용하지 않겠다는 뜻입니다.
...
```
한 가지 문제가 있는데, 멤버 변수가 포인터라면 포인터의 복제가 가능합니다.<br>
포인터가 가지고있는 주소값은 상수로 내보낼 수 없지만 포인터가 가리키는 변수의 값은 멤버로 보지 않는다는 것입니다.<br>
...
//result
20 //10이였던 Password의 값이 20으로 변경되었습니다.
```

#### 상수 객체
클래스 객체 생성시 const 키워드를 사용하면 해당 객체를 상수화 시킬수 있습니다.<br>
상수 객체에 대하여 해당 클래스 내부의 함수는 상수 멤버 함수만 사용할 수 있습니다.
